...
def disagg_prec_cascade(precip_daily,
    ...
    for l in range(1, 6):
        ...
        # evaluate time step
        dt = (precip_daily.index[1] -
              precip_daily.index[0]).total_seconds() / 3600  # hours
        ...
        for i in range(0, len(vdn_in)):
            ...
            if vdn_in[i] > 0:
                ...
                #
                p = np.zeros((3, 1))
                p[0] = cascade_options.p01[belowabove,
                                           vbtype-1]  # index changed!
                p[1] = cascade_options.p10[belowabove,
                                           vbtype-1]
                p[2] = cascade_options.pxx[belowabove,
                                           vbtype-1]

                # draw a random number to determine the branching type
                ...
                if rndp <= p[0]:
                    ...
                    # check results (in the previous version this error
...
def precip_master_station(precip_daily,
    ...
    for index_d, precip in precip_daily.iteritems():
        ...
        if precip_h.sum() != 0 and precip_h.sum() != np.isnan(precip_h.sum()):
            ...
            # uniform option will preserve daily data by uniform
            # distribution
            if zerodiv == 'uniform':
...
def build_casc(hourlyDataObs,
    ...
    for cur_months in months:
        ...
        # statistics object
        if avg_stats:
            # in this case, the average statistics will be applied for
            # all levels likewise
            stat_obj = casc_opt
        else:
            # for longer time series, separate statistics might be more
            # appropriate level dependent statistics will be assumed
